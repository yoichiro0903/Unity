...
public class NekoControl : MonoBehaviour {
	...
	public const float	JUMP_HEIGHT_MAX = 5.0f;								// ジャンプの高さ.
	public const float	JUMP_KEY_RELEASE_REDUCE = 0.5f;						// ジャンプ中にキーを離したときの、上昇速度のスケール.

	public const float	RUN_SPEED_MAX   = 5.0f;								// 走りのスピードの最大値.
	public const float	RUN_ACCELE      = RUN_SPEED_MAX/2.0f;				// 走りのスピードの加速.

	public const float	SLIDE_SPEED_MAX = 2.0f;								// 左右移動のスピード.
	public const float	SLIDE_ACCEL     = SLIDE_SPEED_MAX/0.1f;				// 左右移動の加速度.

	public const float SLIDE_ACCEL_SCALE_JUMP = 0.1f;						// 左右移動の加速度のスケール（ジャンプ中）.

	public const float	RUN_SPEED_DECELE_MISS      = RUN_SPEED_MAX/2.0f;	// ミスした時の減速度.
	public const float	RUN_SPEED_DECELE_MISS_JUMP = RUN_SPEED_MAX/5.0f;	// ミスした時の減速度（ジャンプ中）.

	public static Vector3 COLLISION_OFFSET = Vector3.up*0.2f;

	// ---------------------------------------------------------------- //

	public const float SLIDE_ROTATION_MAX         = 0.2f;						// 左右移動のローテーションスピード.
	public const float SLIDE_ROTATION_SPEED       = SLIDE_ROTATION_MAX/0.1f;	// 左右移動のローテーション加速度.
	public const float SLIDE_ROTATION_COEFFICIENT = 2.0f;						// 左右移動のローテーション加速度の係数.

	public const float JUMP_ROTATION_MAX         = 0.25f;						// 上下のローテーションスピード（ジャンプ中）.
	public const float JUMP_ROTATION_SPEED       = JUMP_ROTATION_MAX/0.1f;		// 上下のローテーション加速度（ジャンプ中）.
	public const float JUMP_ROTATION_COEFFICIENT = 0.25f;						// 上下のローテーション加速度の係数（ジャンプ中）.

	public const float SLIDE_VELOCITY = 1.0f;									// 左右移動のローテーション速度.
	public const float JUMP_VELOCITY  = 4.0f;									// 上下のローテーション速度（ジャンプ中）.
		...
		switch(this.step) {
			...
			case STEP.MISS:
			{
				...
				do {
					...
					// 最初のモーション（failed_1）が１ループ再生されるまで待つ.
